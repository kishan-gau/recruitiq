...
describe('PayrollRunTypeRepository', () => {
  ...
  describe('findAll', () => {
    ...
    it('should filter out inactive run types by default', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining('AND prt.is_active = TRUE'),
        expect.any(Array),
        testOrgId,
        expect.any(Object)
      );
    ...
    it('should generate UUID for new run type', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          expect.stringMatching(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i) // UUID v4 pattern
        ]),
        testOrgId,
        expect.any(Object)
      );
  ...
  describe('update', () => {
    ...
    it('should only update provided fields', async () => {
      ...
      const partialUpdates = {
        typeName: 'New Name' // Only updating name
      };
    ...
    it('should enforce multi-tenant isolation in WHERE clause', async () => {
      // Arrange
      const updates = { typeName: 'Updated' };
      mockQuery.mockResolvedValue({ rows: [] });
  ...
  describe('softDelete', () => {
    it('should soft delete run type by setting deleted_at', async () => {
      // Arrange
      const deletedRunType = createDbRunType({
        deleted_at: new Date(),
      ...
      // Assert
      expect(result).toEqual(deletedRunType);
      expect(result.deleted_at).toBeDefined();
      expect(result.deleted_by).toBe(testUserId);
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE payroll.payroll_run_type'),
        expect.arrayContaining([testRunTypeId, testOrgId, testUserId]),
        testOrgId,
