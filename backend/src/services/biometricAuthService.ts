...
class BiometricAuthService {
  ...
  private rpName: string = 'RecruitIQ';
  private rpID: string;
   ...
   * Validation schemas
   */
  static registerOptionsSchema = Joi.object({
    employeeId: Joi.string().uuid().required(),
    deviceName: Joi.string().max(255).optional(),
  });
  
  static registerVerifySchema = Joi.object({
    employeeId: Joi.string().uuid().required(),
  ...
  static authenticateVerifySchema = Joi.object({
    employeeId: Joi.string().uuid().required(),
    response: Joi.object().required(),
  });
  ...
  async generateRegistrationOptions(
    ...
    try {
      ...
      const employeeResult = await query(
        employeeQuery,
        [validated.employeeId, organizationId],
        organizationId
      );
      ...
      const existingCredentials = await this.getEmployeeCredentials(
  ...
  async verifyAuthentication(
    ...
    try {
      ...
      const credResult = await query(
        credQuery,
        [credentialId, organizationId, validated.employeeId],
        organizationId
      );
      ...
      await query(
        updateQuery,
        [verification.authenticationInfo.newCounter, credential.id, organizationId],
        organizationId,
        {
          operation: 'UPDATE',
          table: 'hris.biometric_credential',
