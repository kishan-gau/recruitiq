...
export async function createJob(req, res, next) {
  try {
    // Convert API request (camelCase) to service format (snake_case)
    const jobData = mapApiToDb(req.body);
    
    const job = await jobService.create(jobData, req.user);
...
export async function updateJob(req, res, next) {
  try {
    ...
    const job = await jobService.update(id, updateData, req.user);
