...
CREATE TABLE IF NOT EXISTS payroll.compensation (
  ...
  -- Worker and Component References (foreign keys added later after tables exist)
  worker_metadata_id UUID,
  pay_component_id UUID,
  
  -- Compensation details (legacy fields for backward compatibility)
  compensation_type VARCHAR(20) CHECK (compensation_type IN ('hourly', 'salary', 'commission', 'bonus')),
  ...
  -- Configuration
  calculation_type VARCHAR(50) CHECK (calculation_type IN ('fixed', 'hourly', 'percentage', 'formula')),
  ...
  CONSTRAINT check_amount_or_rate CHECK (
    ...
    (calculation_type IS NULL AND compensation_type IS NOT NULL)  -- Legacy support
  ),
  ...
  UNIQUE(employee_id, effective_from)  -- Legacy unique constraint
);

COMMENT ON TABLE payroll.compensation IS 'Compensation records - links workers to pay components with specific rates and rules. Supports both legacy (employee-based) and new (component-based) compensation models.';
COMMENT ON COLUMN payroll.compensation.employee_id IS 'References hris.employee(id) - the single source of truth';
COMMENT ON COLUMN payroll.compensation.worker_metadata_id IS 'References worker_metadata - for component-based compensation assignments';
COMMENT ON COLUMN payroll.compensation.pay_component_id IS 'References pay_component - specific compensation component being assigned';
COMMENT ON COLUMN payroll.compensation.amount IS 'Fixed amount per pay period (e.g., $5000/month salary)';
COMMENT ON COLUMN payroll.compensation.rate IS 'Hourly/unit rate (e.g., $25/hour)';
COMMENT ON COLUMN payroll.compensation.percentage IS 'Percentage rate (e.g., 5% for commission)';
COMMENT ON COLUMN payroll.compensation.calculation_type IS 'How to calculate compensation: fixed, hourly, percentage, or formula-based';


-- ================================================================
-- WORKER METADATA (PayLinQ-specific worker data)
-- Integration Test Requirements: Added November 15, 2025
-- ================================================================

-- Worker metadata (PayLinQ-specific worker data linked to HRIS employees)
CREATE TABLE IF NOT EXISTS payroll.worker_metadata (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
-- Indexes for worker_metadata
CREATE INDEX IF NOT EXISTS idx_worker_metadata_org_id ON payroll.worker_metadata(organization_id) WHERE deleted_at IS NULL;
...
-- Foreign keys that reference tables created later in the schema
-- ================================================================

-- Add foreign keys to compensation table (references worker_metadata and pay_component)
DO $$
BEGIN
  -- Add worker_metadata_id foreign key
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'fk_compensation_worker_metadata_id'
      AND table_schema = 'payroll'
      AND table_name = 'compensation'
  ) THEN
    ALTER TABLE payroll.compensation
      ADD CONSTRAINT fk_compensation_worker_metadata_id
      FOREIGN KEY (worker_metadata_id)
      REFERENCES payroll.worker_metadata(id)
      ON DELETE SET NULL;
    RAISE NOTICE '[OK] Added foreign key: compensation -> worker_metadata';
  END IF;

  -- Add pay_component_id foreign key
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'fk_compensation_pay_component_id'
      AND table_schema = 'payroll'
      AND table_name = 'compensation'
  ) THEN
    ALTER TABLE payroll.compensation
      ADD CONSTRAINT fk_compensation_pay_component_id
      FOREIGN KEY (pay_component_id)
      REFERENCES payroll.pay_component(id)
      ON DELETE SET NULL;
    RAISE NOTICE '[OK] Added foreign key: compensation -> pay_component';
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_compensation_worker_metadata ON payroll.compensation(worker_metadata_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_compensation_pay_component ON payroll.compensation(pay_component_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_compensation_org_id ON payroll.compensation(organization_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_compensation_active ON payroll.compensation(is_active) WHERE deleted_at IS NULL;
