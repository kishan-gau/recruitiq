...
export function up(knex) {
  return knex.schema.createTable('forfait_rules', (table) => {
    // Primary key
    ...
    table.foreign('source_component_id').references('id').inTable('pay_components');
    ...
    table.foreign('forfait_component_id').references('id').inTable('pay_components');
  ...
  .then(() => {
    ...
    return knex.schema.alterTable('forfait_rules', (table) => {
  ...
  .then(() => {
    // Add comments
    return knex.raw(`
      COMMENT ON TABLE forfait_rules IS 'Manages forfait (lump sum) payment rules that automatically generate additional payment components';
      COMMENT ON COLUMN forfait_rules.rule_name IS 'Name of the rule (e.g., "5% Vacation Allowance on Bonus")';
      COMMENT ON COLUMN forfait_rules.source_component_id IS 'The pay component that triggers this forfait rule';
      COMMENT ON COLUMN forfait_rules.forfait_component_id IS 'The forfait component to create when rule is triggered';
      COMMENT ON COLUMN forfait_rules.percentage_rate IS 'Percentage to apply (e.g., 5.00 for 5%)';
      COMMENT ON COLUMN forfait_rules.catalog_value IS 'Tax catalog code for reporting (e.g., "025" for vacation pay)';
    `);
  });
}

export function down(knex) {
  return knex.schema.dropTableIfExists('forfait_rules');
}
