...
constructor(database = null) {
  ...
  /**
   * Find forfait component assignment linked to a benefit assignment
   * Used to locate auto-generated forfait assignments when updating/deleting benefit
   * 
   * @param {string} benefitAssignmentId - The benefit assignment ID stored in metadata.linkedBenefitAssignmentId
   * @param {string} organizationId - Organization UUID
   * @returns {Promise<Object|null>} Forfait assignment or null
   */
  async findLinkedForfaitAssignment(benefitAssignmentId, organizationId) {
    const result = await this.query(
      `SELECT 
         ea.*,
         pc.component_name,
       ...
       FROM payroll.employee_pay_component_assignment ea
       INNER JOIN payroll.pay_component pc ON ea.component_id = pc.id
       WHERE ea.metadata->>'linkedBenefitAssignmentId' = $1
         AND ea.organization_id = $2
         AND ea.deleted_at IS NULL
         AND pc.deleted_at IS NULL`,
      [benefitAssignmentId, organizationId],
      organizationId,
      { operation: 'SELECT', table: 'employee_pay_component_assignment' }
    );
  ...
  /**
   * Find all forfait assignments for an employee
   * Used during payroll calculation to process forfait components
   * 
   * @param {string} employeeId - Employee UUID
   * @param {string} organizationId - Organization UUID
   * @param {Date} effectiveDate - Date to check for active assignments
   * @returns {Promise<Array>} Forfait assignments
   */
  async findEmployeeForfaitAssignments(employeeId, organizationId, effectiveDate) {
    const result = await this.query(
      `SELECT 
         ea.*,
         pc.component_name,
       ...
       WHERE ea.employee_id = $1
         ...
         AND ea.metadata->>'autoGenerated' = 'true'
         AND ea.deleted_at IS NULL
         AND pc.deleted_at IS NULL
       ORDER BY ea.effective_from DESC`,
