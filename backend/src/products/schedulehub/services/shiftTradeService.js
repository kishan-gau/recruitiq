...
class ShiftTradeService {
  ...
  async createSwapOffer(offerData, organizationId, userId) {
    ...
    try {
      ...
      const shiftCheck = await client.query(
        `SELECT id, worker_id, shift_date FROM scheduling.shifts 
      ...
      const result = await client.query(
        `INSERT INTO scheduling.shift_swap_offers (
          organization_id, shift_id, offering_worker_id, swap_type,
          target_worker_id, requires_approval, expires_at, reason, status
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        RETURNING *`,
        [organizationId, value.shiftId, shift.worker_id, value.swapType,
    ...
    try {
      ...
      const result = await client.query(
        `INSERT INTO scheduling.shift_swap_requests (
          organization_id, swap_offer_id, requesting_worker_id, status
        ) VALUES ($1, $2, $3, $4)
        RETURNING *`,
  ...
  async acceptSwapRequest(requestId, organizationId, userId) {
    ...
    try {
      ...
      const requestCheck = await client.query(
        `SELECT r.*, o.shift_id, o.offering_worker_id, o.requires_approval
      ...
      if (request.requires_approval) {
        ...
        await client.query(
          `UPDATE scheduling.shifts
           SET worker_id = $1, updated_by = $2
           WHERE id = $3`,
          [request.requesting_worker_id, userId, request.shift_id]
        );
        await client.query(
          `UPDATE scheduling.shift_swap_offers
  ...
  async approveSwap(offerId, organizationId, approverId) {
    ...
    try {
      ...
      const offerCheck = await client.query(
        `SELECT o.*, r.requesting_worker_id
      ...
      await client.query(
        `UPDATE scheduling.shifts
         SET worker_id = $1
         WHERE id = $2`,
        [offer.requesting_worker_id, offer.shift_id]
      );
    ...
    try {
      let query = `
        SELECT o.*, s.shift_date, s.start_time, s.end_time, r.role_name,
               w.first_name || ' ' || w.last_name as offering_worker_name
        FROM scheduling.shift_swap_offers o
        JOIN scheduling.shifts s ON o.shift_id = s.id
        JOIN scheduling.roles r ON s.role_id = r.id
        JOIN scheduling.workers w ON o.offering_worker_id = w.id
      ...
      if (workerId) {
        query += ` AND o.offering_worker_id != $2`;
