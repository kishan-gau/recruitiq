...
class BiometricAuthController {
    constructor() {
         ...
         */
        this.generateRegistrationOptions = async (req, res) => {
            try {
                const { organizationId, employeeId, userId } = req.user;
                const { deviceName } = req.body;
                const result = await this.service.generateRegistrationOptions(employeeId, organizationId, deviceName);
                res.json({
                    success: true,
        ...
        this.verifyRegistration = async (req, res) => {
            try {
                ...
                const result = await this.service.verifyRegistration(employeeId, organizationId, response, expectedChallenge, deviceInfo, userId);
                res.status(201).json({
                    success: true,
                    credential: {
         ...
         */
        this.generateAuthenticationOptions = async (req, res) => {
            try {
                const { organizationId, employeeId } = req.user;
                const result = await this.service.generateAuthenticationOptions(employeeId, organizationId);
                res.json({
                    success: true,
                    options: result.options,
         ...
         */
        this.verifyAuthentication = async (req, res) => {
            try {
                const { organizationId, employeeId } = req.user;
                ...
                const result = await this.service.verifyAuthentication(employeeId, organizationId, response, expectedChallenge);
                res.json({
                    success: true,
         ...
         */
        this.getCredentials = async (req, res) => {
            try {
                const { organizationId, employeeId } = req.user;
                const credentials = await this.service.getEmployeeCredentials(employeeId, organizationId);
         ...
         */
        this.revokeCredential = async (req, res) => {
            try {
                const { organizationId, userId } = req.user;
                const { credentialId } = req.params;
                const result = await this.service.revokeCredential(credentialId, organizationId, userId);
                res.json({
                    success: true,
                    credential: {
                        id: result.id,
