...
function validateSecret(secretName, value, definition, environment) {
    ...
    if (definition.minLength && value.length < definition.minLength) {
        const message = `Secret ${secretName} is too short!\n` +
    ...
    if (definition.forbiddenValues) {
        ...
        const forbidden = definition.forbiddenValues.find(f => lowerValue.includes(f.toLowerCase()));
        if (forbidden) {
            const message = `Secret ${secretName} contains forbidden value: "${forbidden}"\n` +
    ...
    if (weakMatch && environment !== 'development') {
        logger.warn(`WARNING: Secret ${secretName} matches weak pattern: ${weakMatch.name}\n` +
...
async function loadFromBarbican(environment) {
    ...
    try {
        ...
        const { default: BarbicanProvider } = await import('./providers/barbicanProvider');
