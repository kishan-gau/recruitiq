...
class BarbicanProvider {
    ...
    validateConfig() {
        ...
        if (missing.length > 0) {
            throw new Error(`Barbican configuration incomplete. Missing: ${missing.join(', ')}\n` +
    ...
    async authenticate() {
        ...
        try {
            ...
            const response = await axios.post(`${this.config.authEndpoint}/auth/tokens`, {
                auth: {
                    identity: {
                        methods: ['password'],
                        password: {
    ...
    async storeSecret(secretName, secretValue, options = {}) {
        try {
            ...
            const response = await axios.post(`${this.config.endpoint}/secrets`, {
                name: secretName,
                payload: secretValue,
