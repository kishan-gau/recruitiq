...
class GeofencingService {
     ...
     */
    async getLocationGeofenceConfig(locationId, organizationId) {
        try {
            const locationQuery = `
        SELECT 
          location_name,
      ...
      `;
            const result = await query(locationQuery, [locationId, organizationId], organizationId);
     ...
     */
    async getEmployeeGeofenceConfig(employeeId, organizationId) {
        try {
      ...
      `;
            const result = await query(employeeQuery, [employeeId, organizationId], organizationId);
     ...
     */
    async validateClockInLocation(employeeId, organizationId, employeeLocation) {
        try {
            // Get geofence configuration
            const geofenceConfig = await this.getEmployeeGeofenceConfig(employeeId, organizationId);
     ...
     */
    async updateLocationGeofence(locationId, organizationId, config, userId) {
        try {
            // Validate config
     ...
     */
    isWithinGeofence(employeeLocation, geofenceCenter, radiusMeters) {
 ...
 * Validation schemas
 */
GeofencingService.locationSchema = Joi.object({
    latitude: Joi.number().min(-90).max(90).required(),
    longitude: Joi.number().min(-180).max(180).required(),
});
GeofencingService.geofenceConfigSchema = Joi.object({
