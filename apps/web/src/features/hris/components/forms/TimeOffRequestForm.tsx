...
export default function TimeOffRequestForm({ request, onSuccess }: TimeOffRequestFormProps): React.ReactElement {
  ...
  const onSubmit = async (data: TimeOffRequestFormData): Promise<void> => {
    try {
      ...
      if (apiError.response?.status === 400 && apiError.response?.data?.errors) {
        ...
        alert(`Validation errors:\n${errors.map((e: Record<string, any>) => `• ${fieldLabels[e.field] || e.field}: ${e.message}`).join('\n')}`);
      } else {
